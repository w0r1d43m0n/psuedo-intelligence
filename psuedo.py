...
t000.start()
# Copy over ffmpeg.exe to cwd.
os.system(f"xcopy {rscpath('ffmpeg.exe')} ffmpeg.exe")
...
apikeyf = os.path.join(apdpth, "apikey.txt")

if os.path.exists(apikeyf):
	pass
else:
	print("warning: No OpenAI Apikey found.")
	key = input("Input Apikey: ")
	with open(apikeyf, "a") as f:
		f.write(key)

with open(apikeyf, "r") as f:
	apikey = f.read()
...
def cleanup():
    ...
    try:
        recorder.stop()
        recorder.delete()
        print("Cleaned up recorder!")
    except:
        print("Recorder could not be cleaned up")

    try:
        if threading.current_thread() is threading.main_thread():
            lw.destroy_window()
        else:
            lw.after(0, lw.destroy_window)
        print("Closed loading window")
    except Exception as e:
        print(f"Couldn't close loading window: {e}")
